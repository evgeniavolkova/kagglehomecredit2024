...
VERSION = "3"
with open(f'{PATH_FEATURES}/features3.txt', 'r') as file:
    ...
    def transform(self, df: pl.DataFrame, agg: bool = True) -> pl.DataFrame:
        ...
        if (depth > 0) & agg:
            ...
            if "creditbureau" in self.name:
                ...
                approved_cols = [i for i in num_cols if i.split("_")[-1] == "dpd"]
                refused_cols = [i for i in num_cols if i.split("_")[-1] not in ["dpd", "active", "closed"]]
                expr_ls = []
                for col1, col2 in zip(approved_cols, refused_cols):
                    expr_ls.append((pl.col(col1) - pl.col(col2)).alias(f"{col1}_mns_{col2}"))
                df = df.with_columns(expr_ls)
