...
class BatchDataHandler:
    ...
    MERGE_COLS_S1 = ["credamount_770A_1_static0"]
...
class FeatureEngineer:
    ...
    def transform(self, df: pl.DataFrame, agg: bool = True) -> pl.DataFrame:
        ...
        col_sort = None
        if self.name == "taxregistrya1":
            col_sort = "recorddate_4527225D_1"
        elif self.name == "taxregistryb1":
            col_sort = "deductiondate_4917603D_1"
        elif self.name == "taxregistryc1":
            col_sort = "processingdate_168D_1"
        elif self.name == "applprev1":
            col_sort = "creationdate_885D_1"
        ...
        if (depth > 0) & agg:
            ...
            if self.name == "applprev1":
                dfx = df.filter(pl.col("approvaldate_319D_1").is_not_null())
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_approved")
                dfs.append(dfi)
                dfx = df.filter(pl.col("approvaldate_319D_1").is_null())
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_refused")
                dfs.append(dfi)
            ...
            for date in self.df_props["columns_date_index"]:
                if f"{date}_1" not in df.columns:
                    continue
                if self.name == "creditbureau2":
                    lengths = [30*12, 30*1]
                    suffixes = ["1y", "1m"]
                else:
                    lengths = [365*5]
                ...
                for length, suffix in zip(lengths, suffixes):
                    dfx = df.filter(pl.col(f"{date}_1") >= -length)
                    dfi = self._process_aggregations_all(dfx, suffix=f"_{date}_{suffix}")
                    dfs.append(dfi)
            ...
            if "creditbureau" in self.name:
                ...
                num_cols = [i for i in num_cols if i not in [COL_ID, COL_DATE, COL_TARGET, COL_WEEK]]
                approved_cols = [i for i in num_cols if i.split("_")[-1] == "active"]
                refused_cols = [i for i in num_cols if i.split("_")[-1] == "closed"]
                expr_ls = []
                for col1, col2 in zip(approved_cols, refused_cols):
                    expr_ls.append((pl.col(col1) / pl.col(col2)).alias(f"{col1}_div_{col2}"))
                df = df.with_columns(expr_ls)
    ...
    def _process_applprev1(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            (pl.col("dtlastpmt_581D_1") - pl.col("firstnonzeroinstldate_307D_1")).alias("dtlastpmt_581D_2"),
            (pl.col("dtlastpmt_581D_1") - pl.col("creationdate_885D_1")).alias("dtlastpmt_581D_3"),
        )
        df = df.with_columns(
            (pl.col("mainoccupationinc_437A_1").fill_null(pl.col("mainoccupationinc_384A_1_person1"))).alias("mainoccupationinc_437A_2")
        )
        df = df.with_columns(
            (pl.col("credacc_credlmt_575A_1") / pl.col("credacc_actualbalance_314A_1")).alias("credacc_credlmt_575A_2"),
            (pl.col("currdebt_94A_1") / pl.col("mainoccupationinc_437A_1")).alias("currdebt_94A_2"),
            (pl.col("annuity_853A_1") / pl.col("mainoccupationinc_437A_1")).alias("annuity_853A_2"),
            (pl.col("outstandingdebt_522A_1") / pl.col("mainoccupationinc_437A_1")).alias("outstandingdebt_522A_2"),
            (pl.col("credamount_590A_1") / pl.col("mainoccupationinc_437A_1")).alias("credamount_590A_2"),
            (pl.col("annuity_853A_1") / pl.col("credamount_590A_1")).alias("annuity_853A_3"),
            (pl.col("downpmt_134A_1") / pl.col("credamount_590A_1")).alias("downpmt_134A_2"),
            ((pl.col("annuity_853A_1") * pl.col("tenor_203L_1") / pl.col("credamount_590A_1") - 1) / pl.col("tenor_203L_1")).alias("annuity_853A_4"),
            (pl.col("credamount_590A_1") / pl.col("credamount_770A_1_static0")).alias("credamount_590A_3"),
        )
        return df
    ...
    def _process_creditbureau1(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            ...
            (pl.col("dpdmax_139P_1")/pl.col("numberofoverdueinstls_725L_1")).alias("dpdmax_139P_2"),
            (pl.col("dpdmax_757P_1") / pl.col("numberofoverdueinstls_834L_1")).alias("dpdmax_757P_2"),
            ...
            (pl.col("outstandingamount_362A_1") / pl.col("totalamount_996A_2")).alias("outstandingamount_362A_2"),
            (pl.col("overdueamount_31A_1") / pl.col("totalamount_6A_2")).alias("overdueamount_31A_2"),
            (pl.col("overdueamount_659A_1") / pl.col("totalamount_996A_2")).alias("overdueamount_659A_2"),
    ...
    def _process_creditbureau2(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            pl.when(pl.col("pmts_dpd_1073P_1")>7).then(1).otherwise(0).alias("pmts_dpd_1073P_2"),
            pl.when(pl.col("pmts_dpd_303P_1")>7).then(1).otherwise(0).alias("pmts_dpd_303P_2"),
            pl.when(pl.col("pmts_dpd_1073P_1")>15).then(1).otherwise(0).alias("pmts_dpd_1073P_3"),
            pl.when(pl.col("pmts_dpd_303P_1")>15).then(1).otherwise(0).alias("pmts_dpd_303P_3"),
        )
        return df
    ...
    def _process_static0(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            ...
            pl.coalesce(pl.col("maininc_215A_1"), pl.col("mainoccupationinc_384A_1_person1")).alias("maininc_215A_2"),
        )

        df = df.with_columns(
            (pl.col("totalsettled_863A_1") / pl.col("totaldebt_9A_1")).alias("totalsettled_863A_2"),
            (pl.col("annuity_780A_1") / pl.col("maininc_215A_2")).alias("annuity_780A_2"),
            (pl.col("annuitynextmonth_57A_1") / pl.col("mainoccupationinc_384A_1_person1")).alias("annuitynextmonth_57A_2"),
            (pl.col("annuity_780A_1") / pl.col("credamount_770A_1")).alias("annuity_780A_3"),
            (pl.col("annuitynextmonth_57A_1") / pl.col("credamount_770A_1")).alias("annuitynextmonth_57A_3"),
            (pl.col("annuity_780A_1") / pl.col("avginstallast24m_3658937A_1")).alias("annuity_780A_4"),
            (pl.col("avglnamtstart24m_4525187A_1") / pl.col("credamount_770A_1")).alias("avglnamtstart24m_4525187A_2"),
            (pl.col("avgoutstandbalancel6m_4187114A_1") / pl.col("credamount_770A_1")).alias("avgoutstandbalancel6m_4187114A_2"),
            (pl.col("annuity_780A_1") / pl.col("avgpmtlast12m_4525200A_1")).alias("annuity_780A_5"),
            (pl.col("credamount_770A_1") / pl.col("maininc_215A_2")).alias("credamount_770A_2"),
            (pl.col("currdebt_22A_1") / pl.col("maininc_215A_2")).alias("currdebt_22A_2"),
            (pl.col("datelastunpaid_3546854D_1") - pl.col("datefirstoffer_1144D_1")).alias("datelastunpaid_3546854D_2"),
            (pl.col("disbursedcredamount_1113A_1") / pl.col("credamount_770A_1")).alias("disbursedcredamount_1113A_2"),
            (pl.col("downpmt_116A_1") / pl.col("credamount_770A_1")).alias("downpmt_116A_2"),
            ...
            (pl.col("maininc_215A_1") / pl.col("mainoccupationinc_384A_1_person1")).alias("maininc_215A_3"),
            ...
            (pl.col("maxoutstandbalancel12m_4187113A_1") / pl.col("credamount_770A_1")).alias("maxoutstandbalancel12m_4187113A_2"),
            ...
            (pl.col("price_1097A_1") / pl.col("maininc_215A_2")).alias("price_1097A_2"),
            (pl.col("sumoutstandtotal_3546847A_1") / pl.col("maininc_215A_2")).alias("sumoutstandtotal_3546847A_2"),
            (pl.col("sumoutstandtotal_3546847A_1") / pl.col("credamount_770A_1")).alias("sumoutstandtotal_3546847A_3"),
            (pl.col("totinstallast1m_4525188A_1") / pl.col("credamount_770A_1")).alias("totinstallast1m_4525188A_2"),
            (pl.col("totinstallast1m_4525188A_1") / pl.col("maininc_215A_2")).alias("totinstallast1m_4525188A_3"),
        )
        return df
    ...
    def _process_staticcb0(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # df = self._scale_amount(df, "mainoccupationinc_384A_1_person1")
        df = df.with_columns(
            (pl.col("pmtssum_45A_1") / pl.col("mainoccupationinc_384A_1_person1")).alias("pmtssum_45A_2"),
        )
        return df
    ...
    def _process_aggregations_all(self, df: pl.DataFrame, cols: list | None = None, suffix: str = "") -> pl.DataFrame:
        ...
        expr_list.append(pl.col(COL_ID).count().alias(f"{COL_ID}_1_count_{suffix}"))
        if self.name in ["creditbureau1", "creditbureau2"]:
            expr_list.append(pl.lit(self.source).alias(f"source_1_{suffix}"))
...
class FinalDataProcessor:
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            (pl.sum_horizontal([
                "amount_4527230A_1_mean_taxregistrya1",
                "amount_4917619A_1_mean_taxregistryb1",
                "pmtamount_36A_1_mean_taxregistryc1"
            ])/3).alias("amount_1_mean_taxregistry1"),
            pl.min_horizontal([
                "amount_4527230A_1_min_taxregistrya1",
                "amount_4917619A_1_min_taxregistryb1",
                "pmtamount_36A_1_min_taxregistryc1"
            ]).alias("amount_1_min_taxregistry1"),
            pl.max_horizontal([
                "amount_4527230A_1_max_taxregistrya1",
                "amount_4917619A_1_max_taxregistryb1",
                "pmtamount_36A_1_max_taxregistryc1"
            ]).alias("amount_1_max_taxregistry1"),
            pl.max_horizontal([
                "amount_4527230A_1_last_taxregistrya1",
                "amount_4917619A_1_last_taxregistryb1",
                "pmtamount_36A_1_last_taxregistryc1"
            ]).alias("amount_1_last_taxregistry1"),

            (pl.sum_horizontal([
                "amount_4527230A_1_mean_1month_taxregistrya1",
                "amount_4917619A_1_mean_1month_taxregistryb1",
                "pmtamount_36A_1_mean_1month_taxregistryc1"
            ])/3).alias("amount_1_mean_1month_taxregistry1"),
            pl.min_horizontal([
                "amount_4527230A_1_min_1month_taxregistrya1",
                "amount_4917619A_1_min_1month_taxregistryb1",
                "pmtamount_36A_1_min_1month_taxregistryc1"
            ]).alias("amount_1_min_1month_taxregistry1"),
            pl.max_horizontal([
                "amount_4527230A_1_max_1month_taxregistrya1",
                "amount_4917619A_1_max_1month_taxregistryb1",
                "pmtamount_36A_1_max_1month_taxregistryc1"
            ]).alias("amount_1_max_1month_taxregistry1"),
            pl.max_horizontal([
                "amount_4527230A_1_last_1month_taxregistrya1",
                "amount_4917619A_1_last_1month_taxregistryb1",
                "pmtamount_36A_1_last_1month_taxregistryc1"
            ]).alias("amount_1_last_1month_taxregistry1"),
        )
