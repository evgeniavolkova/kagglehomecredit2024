...
VERSION = "6"
with open(f'{PATH_FEATURES}/features6.txt', 'r') as file:
...
class Utils:
    ...
    """
    @staticmethod
    def cast_dtypes(df: pl.DataFrame, schema: dict) -> pl.DataFrame:
        """
    ...
    @staticmethod
    def match_cols(dfs: list[pl.DataFrame]) -> list[pl.DataFrame]:
        """
...
class BatchReader:
    ...
    def _match_cols(self, df: pl.DataFrame, file_name: str) -> pl.DataFrame:
        """
        Match the columns of the DataFrame with the schema of the dataset.
        (Required for concatenation of different sources of data).
...
class BatchDataHandler:
    ...
    MERGE_COLS_S1 = ["credamount_770A_1_static0", "monthsannuity_845L_1_static0", "credtype_322L_1_static0", "maininc_215A_1_static0"]
...
class FeatureEngineer:
    ...
    def transform(self, df: pl.DataFrame, agg: bool = True) -> pl.DataFrame:
        ...
        if (depth > 0) & agg:
            ...
            if self.name == "applprev1":
                dfx = df.filter(pl.col("credtype_322L_1_static0")==pl.col("credtype_587L_1"))
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_samecredtype")
                dfs.append(dfi)

                dfx = df.filter(pl.col("maxdpdtolerance_577P_5")>0)
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_dpd")
                dfs.append(dfi)
            ...
            if self.name == "creditbureau1":
                dfx = df.filter(pl.col("dpdmax_all_5")>0)
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_dpd")
                dfs.append(dfi)

            if self.name == "creditbureau2":
                dfx = df.filter(pl.col("pmts_dpd_all_5")>0)
                dfx = dfx.select(dfx.select(pl.col(pl.NUMERIC_DTYPES)).columns)
                dfi = self._process_aggregations_all(dfx, suffix="_dpd")
                dfs.append(dfi)
            ...
            if "taxregistry" in self.name:
                ...
                if "taxregistrya1" in self.name:
                    dfx = df.filter(pl.col("recorddate_4527225D_1")>=0)
                if "taxregistryb1" in self.name:
                    dfx = df.filter(pl.col("deductiondate_4917603D_1")>=0)
                if "taxregistryc1" in self.name:
                    dfx = df.filter(pl.col("pmtamount_36A_1")>=0)
                dfi = self._process_aggregations_all(dfx, suffix="_future")
                dfs.append(dfi)
            ...
            for date in self.df_props["columns_date_index"]:
                if f"{date}_1" not in df.columns:
                    continue
                if self.name == "creditbureau2":
                    lengths = [30*12, 30*1, 30*6, 365*2, 365*3]
                    suffixes = ["1y", "1m", "6m", "2y", "3y"]
                else:
                    lengths = [365*7, 365*3]
                    suffixes = ["7y", "3y"]
    ...
    def _process_applprev1(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            (pl.col("annuity_853A") / pl.col("mainoccupationinc_437A_1")).alias("annuity_853A_2"),
            (pl.col("annuity_853A") / pl.col("credamount_590A_1")).alias("annuity_853A_3"),
            (pl.col("annuity_853A") / pl.col("monthsannuity_845L_1_static0")).alias("annuity_853A_4"),
            ...
            (pl.col("credacc_credlmt_575A") / pl.col("credamount_590A_1")).alias("credacc_credlmt_575A_2"),
            (pl.col("credacc_credlmt_575A") + pl.col("credamount_590A_1")).alias("credamount_590A_2"),
            (pl.col("credacc_maxhisbal_375A") / pl.col("credacc_credlmt_575A")).alias("credacc_maxhisbal_375A_2"),
            (pl.col("credacc_minhisbal_90A") / pl.col("credacc_credlmt_575A")).alias("credacc_minhisbal_90A_2"),
            (pl.col("credamount_590A_1") / pl.col("mainoccupationinc_437A_1")).alias("credamount_590A_3"),
            (pl.col("credamount_590A_1") / pl.col("credamount_770A_1_static0")).alias("credamount_590A_4"),
            (pl.col("downpmt_134A") / pl.col("mainoccupationinc_437A_1")).alias("downpmt_134A_2"),
            (pl.col("outstandingdebt_522A") / pl.col("credamount_590A_1")).alias("outstandingdebt_522A_1"),
            (pl.col("pmtnum_8L_1") / pl.col("tenor_203L_1")).alias("pmtnum_8L_2"),

            (pl.col("credacc_credlmt_575A") / pl.col("credacc_actualbalance_314A_1")).alias("credacc_credlmt_575A_3"),
            (pl.col("currdebt_94A_1") / pl.col("mainoccupationinc_437A_1")).alias("currdebt_94A_2"),
            (pl.col("outstandingdebt_522A") / pl.col("mainoccupationinc_437A_1")).alias("outstandingdebt_522A_2"),
            
            (pl.col("downpmt_134A") / pl.col("credamount_590A_1")).alias("downpmt_134A_3"),
            ((pl.col("annuity_853A") * pl.col("tenor_203L_1") / pl.col("credamount_590A_1") - 1) / pl.col("tenor_203L_1")).alias("annuity_853A_5"),
            (pl.col("credamount_590A_1") / pl.col("credamount_770A_1_static0")).alias("credamount_590A_5"),
    ...
    def _process_creditbureau1(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns(
            (pl.col("totalamount_996A_2") / pl.col("credamount_770A_1_static0")).alias("totalamount_996A_3"),
            (pl.col("totalamount_6A_2") / pl.col("credamount_770A_1_static0")).alias("totalamount_6A_3"),
...
class DataProcessor6:
    ...
    def get_data(self, test: bool = False, fit: bool = False, load: bool = False, case_ids = None, save_to_disc: bool = True) -> pd.DataFrame:
        ...
        Arguments:
            test: Specifies whether to load test or train data.
